...
export async function GET(req: NextRequest) {
    try {
        ...
        if (role !== "company") {
            return NextResponse.json(
                { success: false, message: "Unauthorized access: Browsing vehicles is restricted to company accounts" },
                { status: 403 }
            );
        }
        ...
        if (noPagination) {
            ...
            return NextResponse.json(
                { success: true, data: vehicles, count: vehicles.length, total: vehicles.length },
                { status: 200 }
            );
        }
        ...
        return NextResponse.json(
            {
                success: false,
                message: "Server error occurred while fetching vehicles",
                error: error.message,
            },
            { status: 500 }
        );
