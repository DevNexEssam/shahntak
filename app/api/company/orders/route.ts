...
export async function GET(req: NextRequest) {
    try {
        ...
        if (role !== "company") {
            return NextResponse.json(
                { success: false, message: "غير مصرح لك: تصفح الطلبات مخصص لحسابات الشركات فقط" },
                { status: 403 }
            );
        }
        ...
        if (search) {
            filter.$or = [
                ...
                { recipientCity: { $regex: search, $options: "i" } },
            ];
        }
        ...
        return NextResponse.json(
            {
                success: false,
                message: "حدث خطأ في الخادم أثناء جلب الطلبات",
                error: error.message,
            },
            { status: 500 }
        );
