...
export async function GET(_req: Request, context: any) {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return NextResponse.json({ success: false, message: "معرف بوليصة الشحن غير صالح" }, { status: 400 });
        }

        const waybill = await Waybill.findById(id).populate("shipmentId", "shipmentNumber status origin destination");
        if (!waybill) {
            return NextResponse.json({ success: false, message: "بوليصة الشحن غير موجودة" }, { status: 404 });
        }
...
export async function DELETE(_req: Request, context: any) {
    try {
        ...
        if (!deleted) {
            return NextResponse.json({ success: false, message: "بوليصة الشحن غير موجودة أو تم حذفها سابقاً" }, { status: 404 });
        }

        return NextResponse.json({ success: true, message: "تم حذف بوليصة الشحن بنجاح" }, { status: 200 });
    } catch (error: any) {
        return NextResponse.json({ success: false, message: "حدث خطأ في الخادم، يرجى المحاولة لاحقاً", error: error.message }, { status: 500 });
    }
}
