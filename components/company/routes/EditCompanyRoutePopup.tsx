...
import { useUpdateCompanyRoute } from '@/hooks/company/useCompanyRoute';
...
interface EditCompanyRoutePopupProps {
    isOpen?: boolean;
    routeData: any | null;
    onClose: () => void;
}

export default function EditCompanyRoutePopup({ isOpen = true, routeData, onClose }: EditCompanyRoutePopupProps) {
    ...
    const { mutate: updateRoute, isPending: isSubmitting } = useUpdateCompanyRoute();

    useEffect(() => {
        if (routeData) {
            const rawCarrierId = typeof routeData.carrierId === 'object' && routeData.carrierId !== null
                ? routeData.carrierId._id
                : (routeData.carrierId || '');
            ...
            setFormValues({
                ...
                basePrice: routeData.basePrice || 0,
                carrierId: rawCarrierId,
                estimatedTransitTime: routeData.estimatedTransitTime || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        updateRoute(
            { id: routeData._id, updates: validation.data },
            {
                onSuccess: () => {
                    onClose();
                },
