...
export default function CompanyShipments() {
    ...
    // 3. Custom React Query Hook for Company Shipments
    const { data: responseData, isLoading, isError, error, isFetching, refetch } = useCompanyShipments(page, limit, searchQuery);
                        ...
                        <table className="w-full text-right text-sm border-collapse">
                            <thead>
                                <tr className="bg-surface-muted/60 border-b border-border text-xs font-bold text-body">
                                    ...
                                    <th className="py-3.5 px-4">التكلفة / سعر العميل</th>
                                ...
                                {shipmentsList.map((shp: any) => {
                                    ...
                                    return (
                                        <tr key={shp._id} className="hover:bg-surface-muted/40 transition-colors">
                                            ...
                                            <td className="py-3.5 px-4 font-latin text-xs">
                                                <div className="font-bold text-emerald-600">{shp.customerPrice || 0} ر.س</div>
                                                <div className="text-body text-[11px]">تكلفة: {shp.shippingCost || 0} ر.س</div>
                                            </td>
