...
export default function DetailsCompanyUsers({
    ...
    // Fetch live user data & ordersCount
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h2 className="text-xl font-extrabold text-heading">
                                    {user?.userName || "Employee Details"}
                                </h2>
                                {isFetching && (
                                    <LuRefreshCw className="w-3.5 h-3.5 animate-spin text-accent" title="Refreshing data..." />
                                )}
                            </div>
                            <p className="text-xs text-body mt-0.5">Quick preview of the employee's data and registered orders</p>
                        </div>
                    </div>
                ...
                <div className="p-6 overflow-y-auto space-y-5 flex-1 text-left">
                    ...
                    {isLoading && !user ? (
                        <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
                            ...
                            <p className="text-xs font-bold text-body">Loading employee details...</p>
                        </div>
                    ) : user ? (
                        <>
                            ...
                            <div className="p-4 rounded-2xl bg-surface-muted border border-border flex items-center justify-between flex-wrap gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        title={user.userIsActive ? "Click to deactivate account" : "Click to activate account"}
                                    >
                                        <LuPower className={`w-3.5 h-3.5 ${isToggling ? "animate-spin" : ""}`} />
                                        <span>{user.userIsActive ? "Active (Deactivate?)" : "Inactive (Activate?)"}</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-surface rounded-2xl border border-border flex items-center justify-between">
                                    <div>
                                        <span className="text-xs font-semibold text-body block mb-1">Orders Created by Employee</span>
                                        <h4 className="text-2xl font-bold text-heading font-latin">{ordersCount} <span className="text-xs font-normal text-body">orders</span></h4>
                                    </div>
                                ...
                                <div className="p-4 bg-surface rounded-2xl border border-border flex items-center justify-between">
                                    <div>
                                        <span className="text-xs font-semibold text-body block mb-1">Contact Phone Number</span>
                                        <h4 className="text-sm font-bold text-heading font-latin">{user.phone || 'Not registered'}</h4>
                                    </div>
                                    <div className="w-10 h-10 rounded-2xl bg-accent/10 text-accent flex items-center justify-center shrink-0">
                            ...
                            <div className="p-4 rounded-2xl bg-surface border border-border space-y-2">
                                ...
                                <div className="flex items-center justify-between text-sm pt-1">
                                    <div>
                                        <span className="text-xs text-body block font-medium">Company Name:</span>
                            ...
                            <div className="space-y-2">
                                ...
                                {user.permissions && user.permissions.length > 0 ? (
                                    ...
                                    <p className="text-xs text-body italic">No additional permissions assigned (default role permissions apply)</p>
                                )}
                            </div>
                            ...
                            <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-body">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>Added on: {user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US') : 'Not specified'}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
