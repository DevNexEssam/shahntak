            ...
            <div className="relative w-full max-w-lg bg-surface border border-border rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-border flex items-center justify-between bg-surface-muted/50">
                    <div className="flex items-center gap-3.5">
                        ...
                        <div>
                            <h2 className="text-xl font-extrabold text-heading">تفاصيل المركبة</h2>
                            <p className="text-xs text-body mt-0.5">عرض مواصفات السعة والأوزان وحالة التفعيل</p>
                        </div>
                    </div>
                ...
                <div className="p-6 overflow-y-auto space-y-6 flex-1 text-right">
                    ...
                    <div className="p-5 rounded-2xl bg-surface-muted border border-border flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-xs text-body font-medium block">نوع المركبة</span>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-surface border border-border text-center space-y-1">
                            ...
                            <span className="text-xs text-body font-medium block">الوزن الأقصى (الذروة)</span>
                            <span className="text-base font-extrabold text-heading font-latin">
                                {vehicle.capacityWeight ? `${vehicle.capacityWeight} كجم` : 'غير حدد'}
                            </span>
                        </div>

                        <div className="p-4 rounded-2xl bg-surface border border-border text-center space-y-1">
                            <LuBox className="w-6 h-6 text-accent mx-auto mb-1" />
                            <span className="text-xs text-body font-medium block">الحجم الأقصى (الحجمي)</span>
                            <span className="text-base font-extrabold text-heading font-latin">
                                {vehicle.capacityVolume ? `${vehicle.capacityVolume} م³` : 'غير حدد'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-border flex items-center justify-between text-xs text-body">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>تاريخ الإضافة للأسطول: {vehicle.createdAt ? new Date(vehicle.createdAt).toLocaleDateString('ar-SA') : 'غير محدد'}</span>
                        </div>
                    </div>
