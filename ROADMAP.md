...
### 10. 🏛️ حوكمة ضريبة القيمة المضافة وإنشاء الفواتير وتجميد النسب (ZATCA Tax Governance & Invoice Creation)
- [x] **تطبيق إعدادات الضريبة الرسمية (ZATCA 15% / 0%)**: إضافة نسبة `vatRate` وسبب الإعفاء الرسمي `vatExemptionReason` وإلزامية الرقم الضريبي `taxNumber` لـ 15%.
- [x] **سجل تدقيق الضريبة (`taxRateAuditLog`)**: تسجيل تاريخ وتوقيت كلاً من تغييرات النسبة الضريبية مع إلزامية كتابة سبب التعديل وحفظ معرّف المستخدم.
- [x] **تجميد نسبة الضريبة بالمستند التاريخي (`taxRateSnapshot`)**: تجميد النسبة الضريبية لكل فاتورة وقت الإنشاء لعدم تلاعب الفواتير القديمة بأثر رادعي.
- [x] **إنشاء الفواتير يدوياً وحظر التكرار (Anti-Duplication Guard)**: تصفية الشحنات غير المفوترة في المنسدلة وإلزامية اختيار الشحنة ومنع تكرار الفواتير لنفس الشحنة.
- [x] **الخصم المالي والتصحيح المحاسبي (`discount`)**: دعم تطبيق خصم مالي مباشر مع حسابات الصافي والضريبة المضافة والإجمالي.

---

### 11. 💸 نظام المصروفات التشغيلية والتبويبات المالية (Expenses Management System)
- [x] **بناء موديل ومسارات المصروفات (`models/expense.ts` & `/api/company/expenses`)**: إنشاء نظام معزول لتوثيق نفقات الشركة بالباك والفرونت إند.
- [x] **التصنيفات الجاهزة والمخصصة**: دعم اختيار تصنيفات جاهزة (وقود، صيانة، رواتب، إلخ) مع إمكانية **كتابة تصنيف مخصص يدوياً**.
- [x] **حساب صافي الربح التشغيلي تلقائياً**: حساب صافي الرصيد المالي ($\text{إجمالي الإيرادات} - \text{إجمالي المصروفات}$).

---

### 12. 📊 لوحة التقارير والتحليلات الشاملة بالأربعة تبويبات والتحميل الكسول (4-Tab Interactive Analytics Dashboard)
- [x] **التحميل التدريجي السلس (Tab-Based Lazy Loading & Skeleton UI)**: تحميل المؤشرات العلوية فوراً، وجلب بيانات التبويب النشط فقط عند ضغطه منعاً للثقل، مع هيكل عظمي وهمي متحرك (`CompanyAnalyticsSkeleton`).
- [x] **التبويبات الأربعة المتخصصة (`recharts`)**:
  ...
  3. 💸 **المصروفات (Expenses)**: أعلى بند نفقات، متوسط مصروف الرحلة، ورسم التوزيع الدائري للمصروفات.
  4. 🏛️ **الضريبي (ZATCA Tax)**: ضريبة المبيعات الصادرة، ضريبة المدخلات القابلة للخصم، وصافي الضريبة المستحقة للهيئة.
- [x] **فحص الإنتاج وتجميع TypeScript**: النجاح التام لأمر البناء المباشر (`npm run build`) بنسبة 100% وبدون أي أخطاء (`0 errors`).
...
> **وقفنا في هذه الجلسة عند**: إتمام وتأكيد **نظام إدارة المصروفات التشغيلية ولوحة التقارير والتحليلات الشاملة بالأربعة تبويبات والتحميل التدريجي (Expenses System & 4-Tab Interactive Analytics Dashboard)** بنجاح 100% مع النجاح التام لأمر بناء الإنتاج المباشر (`npm run build` 0 errors).
>
> **الخطوة القادمة المباشرة المقرر تنفيذها (الطباعة)**:
> 🖨️ **تطوير وتخصيص قوالب طباعة الفواتير الضريبية والبوالص**:
> 1. تحسين تنسيقات الطباعة الورقية (`@media print` CSS) للبوالص والفواتير المالية لإبراز الخصم والأنصاب الضريبية والإقرار الرسمي.

---

## 🚀 الخطط والقادم في الجلسات القادمة (Next Session Roadmap Tasks)

### 👑 خطة ومراحل تطوير واجهات ونواقص لوحة الأدمن (`Admin UI/UX Parity Plan`):
- [ ] **المرحلة 1: طلبات الأدمن**: إضافة شريط التصفية بالتواريخ السريعة (`اليوم`, `هذا الشهر`, `جميع الفترات`) مع منقي التاريخ + أزرار تنزيل سندات التسليم PDF مباشرة بكل سطر بالجدول.
- [ ] **المرحلة 2: شحنات الأدمن**: إضافة شريط التصفية السريعة + أزرار تنزيل بوليصة الشحن PDF مباشرة بكل سطر + نافذة استيراد الشحنات الجماعي بـ Excel للشركات `BulkImportAdminShipmentsPopup.tsx`.
- [ ] **المرحلة 3: فواتير الأدمن**: التبويبات الفاصلة للمالية وأزرار تنزيل فاتورة ZATCA الضريبية PDF مباشرة بكل سطر بالجدول.
- [ ] **المرحلة 4: المسارات والأسطول**: مكاملة `AdminCompanySelect & SubscriptionWidget` ونظام كروت وإحصائيات مركبات الأسطول.
- [ ] **المرحلة 5: التصفير والتثبت**: التصفير التلقائي للنماذج والتحقق الفني الشامل بـ `npx tsc --noEmit` واجتياز بناء الإنتاج المباشر `npm run build`.
