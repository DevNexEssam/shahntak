...
export const companyOrderServices = {
    getOrders: async (
        ...
        let url = `/api/company/orders?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`;
    ...
    bulkImportOrders: async (orders: any[]): Promise<any> => {
        const { data } = await axios.post("/api/company/orders/bulk", { orders });
        return data;
    }
};
