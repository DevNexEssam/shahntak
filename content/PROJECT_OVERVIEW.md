...
## 🏛️ 8. أحدث التحديثات المعمارية وحوكمة الأسعار (Recent Updates)

* **حوكمة الأسعار وحظر التعديل اليدوي خارج الفواتير**:
  - تم إلغاء وإخفاء كافة حقول الأسعار والتكاليف اليدوية من نوافذ تجميع الطلبات والشحنات (`GroupCompanyOrdersPopup`, `AddCompanyShipmentPopup`, `EditCompanyShipmentPopup`, `CompanyShipments`, `DetailsCompanyShipmentPopup`).
  - أصبحت صفحة الفواتير (`CompanyInvoices.tsx`) والنافذة المنبثقة للتعديل هي الجهة الوحيدة المخولة بتعديل الأسعار والخصومات بحرية.
* **الحساب والربط التلقائي بأسعار المسارات في الباك إند**:
  - حساب تلقائي لسعر الشحنة والفاتورة ($\text{مجموع مبالغ الطلبات المجمعة} + \text{سعر المسار اللوجستي}$) فور إنشاء الشحنة.
  - المزامنة التلقائية والربط المزدوج بين الشحنة والفاتورة بـ `invoiceId` و `shipmentId` لضمان إظهار رقم بوليصة الشحن الرسمية (Waybill Number) في تفاصيل الفاتورة.
* **إعادة هيكلة صفحات التقارير والمالية**:
  - تنظيف صفحة المعاملات المالية والمصروفات (`CompanyInvoices.tsx`) للتركيز فقط على الإيرادات والمصروفات.
  - نقل لوحة التقارير والتحليلات الشاملة (`CompanyAnalyticsDashboard.tsx`) إلى صفحة التقارير الإحصائية (`CompanyReports.tsx`) كـ Tab رئيسي مخصص للتحليلات الشاملة والزكوية.
* **ميزة استيراد الطلبات والشحنات الجماعية عبر ملفات Excel (`.xlsx` / `.csv`)**:
  - تم بناء نظام الاستيراد الجماعي للطلبات (`app/api/company/orders/bulk/route.ts`) والشحنات المدمجة (`app/api/company/shipments/bulk/route.ts`).
  - توفير نوافذ منبثقة تفاعلية لتنزيل نماذج الإكسل الجاهزة باللغة العربية (`BulkImportOrdersPopup.tsx`, `BulkImportShipmentsPopup.tsx`) وإعادة تصفير النماذج التلقائي (Form Reset).
  - معالجة الذكاء للـ Routes والأسعار وتجميع الطلبات تلقائياً بناءً على رمز التجميع وخطوط السير.
* **توحيد الترقيم التسلسلي الجبري (`ORD-0001` & `SHP-0001`)**:
  - توحيد نظام الترقيم التلقائي ليكون 4 خانات تسلسلية موحدة بدون فجوات عبر لوحات الأدمن والشركات والإنشاء الفردي والاستيراد الجماعي.
* **البناء والإنتاج الفعلي**:
  - تم اجتياز فحص `npx tsc --noEmit` بنجاح بدون أخطاء.
  - تم تشغيل واجتياز البناء الإنتاجي الكامل `npm run build` بنسبة 100% (77/77 صفحة ثنائية وساتاتية وسيرفر).
