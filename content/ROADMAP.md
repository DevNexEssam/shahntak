...
* **إعادة تقييم الفواتير المدفوعة (Paid Invoice Auto Re-evaluation)**: ربط الباك إند (`invoices/[id]/route.ts`) لإعادة حساب حالة الفاتورة وتغييرها من `paid` إلى `issued` (مستحقة) تلقائياً فور تعديل الإجمالي إذا وُجد مبلغ متبقي غير مسدد.
