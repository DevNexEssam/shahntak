...
* ✅ **المرحلة الثالثة: بناء وتنميط واجهات بوابة الشركة (مكتملة بالكامل 100% - 8 أقسام من أصل 8)**:
  ...
  * ✅ **قسم إعدادات الشركة والاشتراك** (`settings`): `CompanySettings.tsx` وإدارة ملف الشركة، حماية الحقول الرسمية في الباك والفرونت إند، وتغيير كلمة المرور بـ `bcrypt`.
* ✅ **المرحلة الرابعة: التنعيم البصري والرسوم البيانية وإصلاحات الاستقرار (Visual Refinements & Recharts Integration) (100%)**:
  * 📊 **دمج Recharts (`recharts`)**: بناء الرسوم البيانية التفاعلية لتدفق الطلبات وحالات الشحنات في Dashboard الشركة بلون الهوية الأساسي `accent` (`#7444fd`).
  * 🎨 **توحيد المظهر البصري**: تطبيق الهوية الدائرية الموحدة لأيقونات كروت الـ KPI (`w-10 h-10 rounded-full bg-accent/10 text-accent`) والأزرار الفلات في جميع أقسام البوابة (الطلبات، الشحنات، الفواتير، الأسطول، الموظفون، الداشبورد).
  * 🐛 **إصلاح خطأ الرندر**: علاج مشكلة `Maximum update depth exceeded` في صفحة الإعدادات.
* ✅ **المرحلة الخامسة: التدقيق الأمني والنزاهة المالية وتأمين الفواتير والبوالص ونظام التعديل (100%)**:
  * 🛡️ **تطبيق الشروط الأمنية الـ 8 بالفواتير والبوالص**:
    1. قفل وتأمين الفواتير المدفوعة `paid` وحظر تعديل مبالغها أو إعادة فتحها.
    2. حظر حذف/إلغاء الفواتير المرتبطة بشحنات مكتملة ومسلّمة `delivered`.
    3. حظر وتعطيل زر طباعة بوليصة الشحن للشحنات الملغاة `cancelled`.
    4. حماية وتصفية تواريخ الإصدار `createdAt/issuedAt` ورقم الفاتورة `invoiceNumber` من أي تغيير.
    5. مطابقة إجمالي الفاتورة `total` تلقائياً مع قيم الطلبات المجمعة بالشحنة الأم `customerPrice`.
    6. توحيد وحساب صافي الخدمة والضريبة المضافة 15% من الباك والفرونت إند.
  * ✏️ **بناء وتنميط نافذة تعديل الفواتير (`EditCompanyInvoicePopup.tsx`)**: إضافة زر التعديل لجدول الفواتير وتوفير إمكانية تعديل تاريخ الاستحقاق `dueDate` والحالة للفواتير غير المدفوعة مع قفل الفواتير المدفوعة.
...
> **وقفنا في هذه الجلسة عند**: إتمام وتطبيق **الدستور الأمني والمالي الشامل لضريبة القيمة المضافة (ZATCA Tax Governance)** بنجاح 100%. يشمل ذلك:
> 1. إتاحة تغيير نسبة الضريبة التشغيلية للشركة (15% أساسية أو 0% معفاة) من شاشة الإعدادات ([CompanySettings.tsx](file:///e:/projects/shahntak/components/company/settings/CompanySettings.tsx)).
> 2. اشتراط وجود الرقم الضريبي (`taxNumber`) لتفعيل 15%، واشتراط اختيار سبب الإعفاء (`vatExemptionReason`) عند اختيار 0%.
> 3. قفل وتجميد النسبة التاريخية بالفاتورة (`taxRateSnapshot`) عزل الفواتير الصادرة والقديمة عن التغيرات المستقبلية.
> 4. اشتراط إدخال سبب التعديل حتمياً وحفظ سجل التدقيق الكامل (`taxRateAuditLog`) واستعراضه بالواجهة.
> 5. توفير نقطة ترقية وترميم البيانات التاريخية ([backfill-invoices](file:///e:/projects/shahntak/app/api/admin/migrations/backfill-invoices/route.ts)).
>
> **الخطوة القادمة المباشرة المقرر تنفيذها (الطباعة)**:
> 🖨️ **تخصيص وتطوير قوالب طباعة الفواتير الضريبية والبوالص**:
> 1. تحديث تنسيقات الطباعة الرسمية للورق والبوالص لتظهر بند الخصم والأنصاب المحاسبية والنسبة التاريخية عند الطباعة.
