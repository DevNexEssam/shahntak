...
## 📌 🎯 الخطوة التنفيذية المحددة للمرة القادمة (Next Session Execution Target)

> [!IMPORTANT]
> **الهدف المباشر للجلسة القادمة**:
> * **استكمال الأقسام الأربعة المتبقية من المرحلة الثالثة لبوابة الشركة (مربوطة بالـ Hooks)**:
>   1. 🚚 **قسم الأسطول والمركبات** (`vehicles`): `CompanyVehicles.tsx` ونماذج الإضافة والتعديل والتفاصيل (`CompanyVehicleServices` + `useCompanyVehicle`).
>   2. 📊 **قسم التقارير والإحصائيات** (`reports`): `CompanyReports.tsx` وتحليلات الأداء.
>   3. ⚙️ **قسم إعدادات الشركة والاشتراك** (`settings`): `CompanySettings.tsx` وإدارة ملف الشركة وحدود الباقة.
>   4. 📊 **الداشبورد الرئيسية للشركة** (`dashboard`): `CompanyDashboard.tsx` والملخص الإحصائي اللحظي.
...
* 🔄 **المرحلة الثالثة: بناء وتنميط واجهات بوابة الشركة (تم إنجاز 50% - 4 أقسام من أصل 8)**:
  ...
  * ✅ **قسم الفواتير والتحصيلات** (`invoices`): `CompanyInvoices.tsx` والتفاصيل المالية وطباعة الفاتورة الضريبية.
  * ✅ **قسم فريق العمل والموظفين** (`employees`): `CompanyEmployees.tsx` ونماذج الإضافة والتعديل والتفاصيل والأدوار (`owner`, `manager`, `staff`).
  * ⏳ **الأقسام الأربعة المتبقية** (`vehicles`, `reports`, `settings`, `dashboard`): هدف الجلسة القادمة.
